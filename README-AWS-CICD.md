...
## 🔐 Authentication via AWS OIDC (OpenID Connect)
...
## 🛠️ How to Perform Your First Deployment
...
## 🔌 Connecting to the Database locally (SQL Access)
Since the database is deployed in a highly secure **Private Subnet**, direct internet access is blocked. 
We have deployed an **EC2 Bastion Host (Jump Box)** to allow secure tunneling without needing SSH keys or opening public ports.

### Step 1: Install the AWS SSM Plugin
Ensure you have the [AWS Systems Manager Session Manager plugin](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html) installed on your local machine.

### Step 2: Establish a Secure Tunnel
Run the following AWS CLI command to open a port-forwarding session. 
*Replace `<BASTION_INSTANCE_ID>` with the actual ID from the EC2 console (it will be named something like `NetworkStack-DataStackBastionHost...`).*
*Replace `<RDS_ENDPOINT>` with your database's endpoint URL (found in the RDS console).*

```bash
aws ssm start-session \
    --target <BASTION_INSTANCE_ID> \
    --document-name AWS-StartPortForwardingSessionToRemoteHost \
    --parameters '{"portNumber":["5432"],"localPortNumber":["5432"],"host":["<RDS_ENDPOINT>"]}'
```

### Step 3: Connect using your SQL Client
Leave the terminal running. Open tools like **DBeaver**, **pgAdmin**, or **DataGrip** and connect using:
- **Host**: `localhost`
- **Port**: `5432`
- **Database Name**: `hospomate`
- **Username / Password**: Retrieve these from AWS Secrets Manager (see previous section).
